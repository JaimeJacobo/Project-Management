...
class ProjectDetails extends Component {
	...
	getSingleProject = () => {
		...
		axios
			.get(`https://project-management-back.herokuapp.com/api/projects/${params.id}`, { withCredentials: true })
	...
	deleteProject = () => {
		const { params } = this.props.match;
		axios
			.delete(`https://project-management-back.herokuapp.com/api/projects/${params.id}`, { withCredentials: true })
			.then(() => {
					...
					this.state.tasks.map((task, index) => {
						return (
							<div key={index}>
								...
								<Link to={`/projects/${this.state._id}/tasks/${task._id}`}>{task.title}</Link>
							</div>
						);
					})}
